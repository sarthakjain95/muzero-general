''' Loads agents given paths to weights and simulates games between those agents '''
...
def simulate_game(game_name, checkpoint1_path, checkpoint2_path):
    ...
    cp1_path = pathlib.Path(checkpoint1_path)
    checkpoint1 = torch.load(cp1_path)

    cp2_path = pathlib.Path(checkpoint2_path)
    checkpoint2 = torch.load(cp2_path)

    seed = numpy.random.randint(10000)
    sim_play = SimulatedPlay(Game, checkpoint1, checkpoint2, config1, config2, seed)
    result = sim_play.play_game(0, 0, False, "agent", 0)
...
class SimulatedPlay:
    ...
    def __init__(self, Game, checkpoint1, checkpoint2, config, config2, seed):
        self.config = config
        ...
        self.model.set_weights(checkpoint1["weights"])
        self.model.to(torch.device("cpu"))
        self.model.eval()

        # Initialize network 2
        self.model2 = models.MuZeroNetwork(self.config2)
        self.model2.set_weights(checkpoint2["weights"])
        self.model2.to(torch.device("cpu"))
        self.model2.eval()
    ...
    def play_game(
        ...
        # Returns total reward for player 0
        p1_reward = sum(
                    reward
