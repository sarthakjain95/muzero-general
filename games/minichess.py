...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (115, 5, 5)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(1225))  # Fixed list of all possible actions. You should only edit the length
        ...
        self.training_steps = 12800  # Total number of training steps (ie weights update according to a batch)
        ...
        # Reset viz content 
        shutil.rmtree("./viz/minichess")
        os.mkdir("./viz/minichess")
...
# Gardner's Chess wrapper on PGX
class GardnersChess:

    '''
        *** Must-do Functions ***
        - [done] step(action:int)
        - [done] to_play()
        - [done] legal_actions()
        - [done] reset()
        - [done, with matplotlib] render()
        - [done] expert_action() # Does not influence training
    '''
    ...
    def render(self):
        ...
        # Convert to PNG
        target_location = f"./viz/minichess/{target_name}"
