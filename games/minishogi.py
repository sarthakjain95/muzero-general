...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.observation_shape = (194, 4, 3)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        self.action_space = list(range(132))  # Fixed list of all possible actions. You should only edit the length
        ...
        self.num_simulations = 36  # Number of future moves self-simulated
        ...
        self.fc_representation_layers = [128, 64]  # Hidden layers in the representation network
        self.fc_dynamics_layers = [128, 32]  # Hidden layers in the dynamics network
        self.fc_reward_layers = [128, 32]  # Hidden layers in the reward network
        self.fc_value_layers = [128, 32]  # Hidden layers in the value network
        self.fc_policy_layers = [128, 32]  # Hidden layers in the policy network

        ### Training
        self.results_path = pathlib.Path(__file__).resolve().parents[1] / "results" / pathlib.Path(__file__).stem / datetime.datetime.now().strftime("%Y-%m-%d--%H-%M-%S")  # Path to store the model weights and TensorBoard logs
        self.save_model = True  # Save the checkpoint in results_path as model.checkpoint
        self.training_steps = 3200  # Total number of training steps (ie weights update according to a batch)
...
class MiniShogi:
    ...
    def render(self):
        ...
        # Convert to PNG
        target_location = f"./viz/minishogi/{target_name}"
